...
/** Create a graph that calculates the new pressure group center positions and
 * momentums with scaling. new_momentum= old_momentum*momentum_scale new_xyz=
 * old_xyz*position_prescale+timestep*momentum*invmass*momentum_prescale*/
class PressureGroupScaleDrift : public CudaIntegratorGraph {
public:
  /** Create a graph to move positions and scale pressure group centers of mass
   * and momentum.*/
  PressureGroupScaleDrift(
      // inputs
      const double3 *__restrict__ momentum_scale, /**< Device Pointer to total
                                                     scaling of momentum.*/
      const double3
          *__restrict__ momentum_prescale, /**< Device pointer to momentum
                                              scaling for position update.*/
      const double3
          *__restrict__ position_prescale, /**< Device pointer to position
                                              scaling before moving.*/
      const double4 *old_momentum_invmass, /**< Device pointer to array of
                                              momentums and inverse masses.*/
      const double4 *old_xyzq, /**< Device pointer to array of positions.*/
      // outputs
...
private:
  ...
  const double3
      *__restrict__ momentum_prescale; /**< Device pointer to momentum scaling
                                          for position update.*/
  const double3 *__restrict__ position_prescale; /**< Device pointer to position
                                                    scaling before moving.*/
  const double4 *old_momentum_invmass; /**< Device pointer to array of momentums
                                          and inverse masses.*/
  const double4 *old_xyzq; /**< Device pointer to array of positions.*/
  // outputs
  double4 *new_momentum_invmass; /**< Device pointer to array of new momentums,
                                    and invers masses.*/
  double4 *new_xyzq; /**< Device pointer to array of new positions.*/
  // parameters
  double timestep; /**< time step.*/
