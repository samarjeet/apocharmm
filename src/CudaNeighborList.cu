...
CudaNeighborList<tilesize>::CudaNeighborList(const CudaTopExcl &topExcl,
  ...
  test = false;
  neighborListSorter = std::make_unique<NeighborListSort>();
  // Won't have registerList in the new version
  // so setting up the cellParam here
  h_cellParam = std::make_shared<CellParam_t>();
  allocate<CellParam_t>(&d_cellParam, sizeof(CellParam_t));

  // p21builder = std::make_shared<CudaP21NeighborListBuild>();
}
...
void CudaNeighborList<tilesize>::registerList(
  ...
  if (filename != NULL) {
    ...
    if (pbc == PBC::P1) {
      builder.push_back(new CudaNeighborListBuild<tilesize>(
          n_int_zone_max, izoneStart, izoneEnd, false));
    } else if (pbc == PBC::P21) {
      builder.push_back(new CudaNeighborListBuild<tilesize>(
          n_int_zone_max, izoneStart, izoneEnd, true));
      // TODO : remove the p21builder
      p21builder = std::make_shared<CudaP21NeighborListBuild>();

    } else {
      std::cerr << "[NeighborList] Only P1 and P21 PBC are supported.\n";
    }
  }
...
void CudaNeighborList<tilesize>::sort(const int indList, const int *zone_patom,
  ...
  sorter.at(indList)->calc_min_max_xyz(zone_patom, xyzq, h_ZoneParam,
                                       d_ZoneParam, stream);
  // int numAtoms = topExcl.get_ncoord(); // TODO : for now
  // neighborListSorter->calcMinMaxXYZ(numAtoms, xyzq, h_cellParam, d_cellParam,
  // stream);
  // --------------------------------------------------------------
  ...
  // neighborListSorter->sortSetup(h_cellParam, d_cellParam);
  // --------------------------------------------------------------
...
void CudaNeighborList<tilesize>::build(const int indList, const int *zone_patom,
  ...
  /*if (pbc == PBC::P21) {
    p21builder->build(
        sorter.at(indList)->get_ncell(), topExcl.getMaxNumExcl(), h_ZoneParam,
        d_ZoneParam, boxx, boxy, boxz, rcut, xyzq, loc2glo,
